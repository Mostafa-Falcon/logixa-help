...
export default function Header() {
  ...
  return (
    ...
    >
        ...
        <div className="flex min-h-18 items-center justify-between gap-4 py-3">
          <Link href="/" className="group flex items-center gap-3">
            <div
              ...
              style={{
                ...
                background: "linear-gradient(135deg, rgba(224, 182, 92, 0.2), rgba(88, 196, 170, 0.14))",
              }}
            >
          ...
          <nav className="hidden items-center gap-2 lg:flex">
            {NAV_LINKS.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Button key={href} asChild variant="ghost" className={`nav-link ${active ? "active bg-white/5 text-white" : "muted"}`}>
                  <Link href={href}>
              ...
              <>
                {profile.role === "admin" && (
                  <Button asChild variant="outline">
                    <Link href="/admin"><ShieldCheck className="h-4 w-4" /> لوحة التحكم</Link>
                  </Button>
                )}
                <div className="surface-soft px-4 py-2 text-sm text-white">{profile.username}</div>
          ...
          <Button type="button" variant="ghost" size="icon" className="lg:hidden" onClick={() => setMenuOpen((c) => !c)}>
      ...
      {menuOpen && (
        <div className="border-t lg:hidden page-fade-in" style={{ borderColor: "rgba(224, 197, 132, 0.12)" }}>
                ...
                {profile ? (
                  <>
                    ...
                    <div className="flex items-center gap-2 px-1">
                      <div className="avatar h-8 w-8 text-xs">{profile.username.slice(0, 1)}</div>
